    ...
    <div class="space-y-4 animate-fade-in">
        ...
        @if($conversations->isEmpty())
            <div class="empty-state">
                ...
                <p class="text-sm text-surface-500 max-w-sm">Les conversations liées à vos réservations apparaîtront ici.</p>
            </div>
        @else
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6" style="height: calc(100vh - 250px);">
                <div class="lg:col-span-1">
                    <div class="bg-white border border-surface-200 rounded-xl shadow-card h-full overflow-hidden flex flex-col">
                        ...
                        <div class="flex-1 overflow-y-auto">
                            @foreach($conversations as $conv)
                                <a href="{{ route('conversations.show', $conv) }}"
                                   class="flex items-center gap-3 p-4 border-b border-surface-50 hover:bg-surface-50 transition-colors duration-150 {{ request()->route('conversation')?->id === $conv->id ? 'bg-navy-50 border-l-2 border-l-navy-600' : '' }}">
                                    <div class="w-9 h-9 rounded-full bg-navy-700 text-white flex items-center justify-center text-xs font-semibold flex-shrink-0">
                ...
                <div class="lg:col-span-2">
                    ...
                    @else
                        <div class="bg-white border border-surface-200 rounded-xl shadow-card h-full flex items-center justify-center">
