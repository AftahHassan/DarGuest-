                    ...
                    @forelse($conv->messages as $msg)
                        ...
                        <div class="flex {{ $isOwn ? 'justify-end' : 'justify-start' }}">
                            <div class="max-w-[75%]">
                                ...
                                @if($msg->aiAnalysis)
                                    <div class="mt-2 p-3 bg-primary-50 rounded-xl border border-primary-100">
                                        <p class="text-[10px] font-semibold text-primary-700 uppercase tracking-wider mb-1">Analyse IA</p>
                                        <div class="flex flex-wrap gap-2">
                                            <x-ui.badge variant="primary" class="!text-[10px]">{{ $msg->aiAnalysis->detected_language }}</x-ui.badge>
                                            <x-ui.badge variant="info" class="!text-[10px]">{{ $msg->aiAnalysis->category }}</x-ui.badge>
                                            @if($msg->aiAnalysis->urgency)
                                                <x-ui.badge variant="danger" dot class="!text-[10px]">Urgent</x-ui.badge>
                                            @endif
                                        </div>
                                    </div>
                                @endif
