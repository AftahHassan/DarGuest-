<x-app-layout>
    @php
        ...
        $property = $conversation->reservation->property;
        $categoryLabels = [
        ...
        $nights = max($conversation->reservation->check_in_date->diffInDays($conversation->reservation->check_out_date), 1);
    ...
    <div class="space-y-6" x-data="{ aiPanel: false }">

        {{-- Back --}}
        <div>
            <a href="{{ route('conversations.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-surface-500 hover:text-surface-900 transition-colors">
        ...
        {{-- Workspace --}}
        <div class="grid grid-cols-1 lg:grid-cols-[340px_minmax(0,1fr)] gap-6 h-[70vh] lg:h-[calc(100vh-16.5rem)] min-h-[520px]">

            {{-- Sidebar --}}
            <div x-data="{ search: '' }" class="panel overflow-hidden flex flex-col min-h-0 hidden lg:flex">
            ...
            <div class="panel overflow-hidden flex flex-col min-h-0">
                <div class="px-4 sm:px-5 py-3 border-b border-surface-200/60 flex items-center justify-between gap-3 bg-white/70 backdrop-blur-sm">
                    <div class="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-xs truncate {{ $conversation->status === 'open' ? 'text-emerald-600 font-medium' : 'text-surface-400' }}">
                                {{ $conversation->status === 'open' ? 'En ligne' : 'Conversation fermée' }} · {{ $property?->title ?? 'Logement' }}
                            </p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0">
                        ...
                        <a href="{{ route('reservations.show', $conversation->reservation) }}"
                ...
                <div x-data="{}" x-init="$nextTick(() => $el.scrollTop = $el.scrollHeight)" class="flex-1 overflow-y-auto chat-bg">
                                        ...
                                        <div class="max-w-[78%] sm:max-w-[70%]">
                                            ...
                                            <div class="msg-bubble-{{ $isAi ? 'ai' : ($isOwn ? 'out' : 'in') }}">
                                                ...
                                                <span class="flex items-center justify-end gap-1 mt-0.5 -mb-0.5 ml-8 text-[10px] {{ $isOwn ? 'text-white/70' : 'text-surface-400' }}">
                                                    ...
                                                    @if($isOwn && $isLast)
                                                        @if($msg->read_at)
                                                            <svg class="w-4 h-4 text-gold-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12.75l5.25 5.25L20.25 5.25"/><path d="M9.75 15.75l1.5 1.5L23.25 4.5" opacity=".55"/></svg>
                                                        @else
                                                            <svg class="w-4 h-4 text-white/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12.75l5.25 5.25L20.25 5.25"/></svg>
                                                        @endif
                                                    @endif
                                                </span>
                                            </div>
                ...
                <div class="px-4 py-3.5 border-t border-surface-200/60 bg-white/70 backdrop-blur-sm">
                    <form method="POST" action="{{ route('conversations.messages.store', $conversation) }}" class="flex items-center gap-3">
                        @csrf
