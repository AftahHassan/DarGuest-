...
<header
    class="fixed top-0 right-0 z-30 h-16 bg-white/90 backdrop-blur-xl border-b border-surface-200/70 transition-all duration-300"
    :class="{
...
>
    <div class="h-16 flex items-center justify-between px-6">
        ...
        <div class="flex items-center gap-1">
            ...
            <x-button href="{{ route('conversations.index') }}" size="icon" variant="ghost">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            <x-button href="{{ route('notifications.index') }}" size="icon" variant="ghost" class="relative">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0"/>
                </svg>
                @php $unreadNotif = $user->notifications()->unread()->count(); @endphp
                @if($unreadNotif > 0)
