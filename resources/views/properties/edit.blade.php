    ...
    <div class="space-y-8">
        ...
        <form method="POST" action="{{ route('properties.update', $property) }}" class="space-y-6" x-data="{ saving: false }" x-on:submit="saving = true">
            ...
            <div class="flex items-center justify-between gap-4" data-aos="fade-up">
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('properties.show', $property) }}" class="px-5 py-2.5 text-sm font-medium text-surface-600 bg-white border border-surface-200 hover:bg-surface-50 hover:text-surface-800 rounded-xl transition-all duration-200 flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                        Annuler
                    </a>
